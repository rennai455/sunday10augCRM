...
async function seed() {
  await pool.query("INSERT INTO agencies (name) VALUES ($1) ON CONFLICT DO NOTHING", ['Demo Agency']);
  const agencyRes = await pool.query("SELECT id FROM agencies WHERE name = $1", ['Demo Agency']);
  ...
  const campaignRes = await pool.query("SELECT id FROM campaigns WHERE name = $1", ['Demo Campaign']);
