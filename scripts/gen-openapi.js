...
openapi.paths['/api/auth/me'] = {
  get: { summary: 'Current user info', responses: { '200': { description: 'OK' }, '401': { description: 'Unauthorized' } } },
};
...
openapi.paths['/api/csrf-token'] = {
  get: { summary: 'Issue CSRF token', responses: { '200': { description: 'OK' } } },
};
...
openapi.paths['/api/leads'] = {
  get: {
    ...
    responses: {
      '200': {
        ...
        headers: { 'X-Total-Count': { schema: { type: 'integer' } } },
        content: {
          'application/json': { schema: { type: 'object', properties: { leads: { type: 'array', items: { $ref: '#/components/schemas/Lead' } } } } },
        },
      },
    },
  },
  post: {
    ...
    requestBody: {
      required: true,
      content: { 'application/json': { schema: { type: 'object', properties: { campaign_id: { type: 'integer' }, name: { type: 'string' }, email: { type: 'string', format: 'email' }, phone: { type: 'string' }, status: { type: 'string' } }, required: ['campaign_id'] } } },
    },
    responses: { '201': { description: 'Created' }, '404': { description: 'Campaign not found' } },
  },
};
openapi.paths['/api/leads/{id}'] = {
  get: { summary: 'Get lead by id', parameters: [{ in: 'path', name: 'id', required: true, schema: { type: 'string', pattern: '^\\d+$' } }], responses: { '200': { description: 'OK' }, '404': { description: 'Not Found' } } },
  put: { summary: 'Update lead', parameters: [{ in: 'path', name: 'id', required: true, schema: { type: 'string', pattern: '^\\d+$' } }], responses: { '200': { description: 'OK' }, '404': { description: 'Not Found' } } },
  delete: { summary: 'Delete lead', parameters: [{ in: 'path', name: 'id', required: true, schema: { type: 'string', pattern: '^\\d+$' } }], responses: { '204': { description: 'No Content' }, '404': { description: 'Not Found' } } },
};
openapi.paths['/api/audit/leads'] = {
  get: {
    summary: 'Recent lead activity (audit)',
    parameters: [ { in: 'query', name: 'limit', schema: { type: 'integer', minimum: 1, maximum: 50 } } ],
    responses: { '200': { description: 'OK' } },
  },
};
