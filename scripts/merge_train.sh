...
run_validation() {
  ...
  curl -sSI "http://localhost:${port}/static/dashboard.html" \
    | grep -i "content-security-policy" \
    | grep -i "nonce-" >/dev/null

  curl -sSI -H "Origin: http://localhost:3000" "http://localhost:${port}/healthz" \
    | grep -i "access-control-allow-origin" >/dev/null || true

  popd >/dev/null
}

run_static_checks() {
  if git grep -n -F "new Pool(" -- ':!db/index.js' ':!scripts/diagnostics.js' >/dev/null 2>&1; then
    log "Multiple Pool instances found"; return 1
  fi
  if git grep -n "process\\.env\\.JWT_SECRET\\s*||" >/dev/null; then
    log "JWT_SECRET fallback detected"; return 1
  fi
  if git grep -n "<script[^>]*>" public/ | grep -v "nonce=" >/dev/null 2>&1; then
    log "Inline <script> without nonce found under public/"; return 1
  fi
  if git grep -n "unsafe-inline" -- Server.js middleware/ config/ 2>/dev/null | grep -q .; then
    log "CSP contains 'unsafe-inline' in server/middleware"; return 1
  fi
}
...
preflight(){
  ...
  if git ls-files | grep -E '(^|/)\\.env($|\\.)|\\.pem$|\\.key$|\\.crt$' \
     | grep -vE '(^|/)\\.env\\.example$' >/dev/null
  then
...
merge_phase(){
  ...
  for pr in "${prs[@]}"; do
    ...
    if [[ "$baseRefName" != "main" || "$mergeable" != "MERGEABLE" || "$isDraft" == "true" ]]; then
      skipped+=("{\"number\":$number,\"title\":\"$title\",\"reason\":\"not mergeable or wrong base\"}")
      continue
    fi
    ...
    local checks_json
    checks_json="$(jq -c '.statusCheckRollup // []' <<<"$meta")"
    if echo "$checks_json" | jq -e 'map((.conclusion // .status // .state // "SUCCESS") | ascii_upcase)
      ...
      skipped+=("{\"number\":$number,\"title\":\"$title\",\"reason\":\"checks failing\"}")
      continue
    fi
    if echo "$checks_json" | jq -e 'map((.status // .state // "COMPLETED") | ascii_upcase)
       | any(.=="PENDING" or .=="IN_PROGRESS")' >/dev/null; then
      skipped+=("{\"number\":$number,\"title\":\"$title\",\"reason\":\"checks pending\"}")
      continue
    fi
...
main(){
  ...
  if [[ "$MERGE_DRY_RUN" == "1" ]]; then
    log "Dry run complete. See $SUMMARY_FILE and $LOG_DIR/"
    exit 0
  fi
  ...
  if jq -e '.failed_validation|length>0' "$SUMMARY_FILE" >/dev/null; then
    gh pr create --base main --head "$TRAIN_BRANCH" \
      --title "Merge train: infra/security + Railway prep" \
      --body "$body" --label merge-train --label infra --label security --draft
  else
    gh pr create --base main --head "$TRAIN_BRANCH" \
      --title "Merge train: infra/security + Railway prep" \
      --body "$body" --label merge-train --label infra --label security
  fi
