...
const CACHE_CONFIG = {
  ...
  API_ROUTES: ['/api/campaigns', '/api/analytics/overview', '/api/performance'],
...
self.addEventListener('push', (event) => {
  ...
  event.waitUntil(self.registration.showNotification('RENN.AI CRM', options));
});
    ...
    if (cacheName.includes('renn-ai')) {
      ...
      for (const request of requests) {
        ...
        if (cacheTime) {
          ...
          const maxAge = cacheName.includes('api')
            ? CACHE_CONFIG.TTL.API
            : CACHE_CONFIG.TTL.STATIC;
