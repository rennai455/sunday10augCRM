...
describe('Leads integration with Postgres', () => {
  ...
  beforeAll(async () => {
    ...
    const a = await pool.query(
      "INSERT INTO agencies (name) VALUES ('Test Agency') RETURNING id"
    );
    ...
    token = jwt.sign({ userId: 123, agencyId, isAdmin: true }, secret, {
      expiresIn: '1h',
    });
  });
  ...
  afterAll(async () => {
    await pool
      .query('DELETE FROM leads WHERE campaign_id = $1', [campaignId])
      .catch(() => null);
    await pool
      .query('DELETE FROM campaigns WHERE id = $1', [campaignId])
      .catch(() => null);
    await pool
  ...
  it('creates, updates, and sorts leads by updated_at', async () => {
    ...
    res = await agent.put(`/api/leads/${id1}`).send({ status: 'contacted' });
  ...
  it('filters leads by date range and sorts by status', async () => {
    ...
    let r = await agent
      .post('/api/leads')
      .send({ campaign_id: campaignId, name: 'C', status: 'alpha' });
    const idA = r.body.id;
    r = await agent
      .post('/api/leads')
      .send({ campaign_id: campaignId, name: 'D', status: 'omega' });
    const idB = r.body.id;

    const now = new Date();
    const past = new Date(now.getTime() - 24 * 3600 * 1000);
    await pool.query('UPDATE leads SET created_at = $1 WHERE id = $2', [
      past,
      idA,
    ]);
    await pool.query('UPDATE leads SET created_at = $1 WHERE id = $2', [
    ...
    let res = await agent.get(
      `/api/leads?from=${encodeURIComponent(from)}&pageSize=10`
    );
    expect(res.status).toBe(200);
